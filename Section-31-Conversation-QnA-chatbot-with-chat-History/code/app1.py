...
st.sidebar.title("Enter Groq API key or Select and LLMs model")
...
if llm:
    ...
    if uploaded_files:
        ...
        text_splitter = RecursiveCharacterTextSplitter(chunk_size=4000, chunk_overlap=450)
        ...
        def get_sesstion_history(session:str)->BaseChatMessageHistory:
            if session_id not in st.session_state.store:
                st.session_state.store[session_id] = ChatMessageHistory()
            return st.session_state.store[session_id]
        
        conversational_reg_chain=RunnableWithMessageHistory(
            rag_chain, get_sesstion_history,
        ...
        if user_input:
            session_history=get_sesstion_history(session_id)
            response = conversational_reg_chain.invoke(
                {"input": user_input},
                config={
                    "configurable": {"session_id": session_id}
                    # constructs as key abc123 in store.
                },
            )
